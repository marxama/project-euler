...
SOLUTION: We can completely disregard n-digit pandigital numbers where
n is in {2,3,5,6,8,9}, due to the fact that numbers whose digits add
up to a number divisible by 3 is itself divisible by 3 - all pandigital
numbers in those groups have this property. We are thus looking to see what
the largest 7- or 4-digit pandigital prime is, assuming there is one -
otherwise, it's simply 7.
Since the last digit of the 7-digit numbers need to be 1, 3 or 7, there are
(* 3 6 5 4 3 2 1) = 2160 numbers to check."
  []
  (->> (range 1 8)
    utils/permutations
    (filter #(#{1 3 7} (last %)))
    (map (fn [c] (->> c (apply str)
                   (#(Integer/parseInt %)))))
    sort
    reverse
    (filter utils/prime?)
...
APPROACH: Backtracking + pruning. Start by finding all 2-3 digit numbers
divisible by 17 (2-digit numbers get prepended by 0). Then, each of those
are checked against all 2-3 digit numbers divisible by 13 - all combinations
where the two 'middle' numbers match and no other digits are shared are kept,
and the process is continued by matching those with numbers divisible by 11,
and so on."
  []
  (let [two-or-three-digit-number? #(< 9 % 1000)
        unique-digits? #(= (count %) (count (distinct %)))
        multiples-of-x-with-unique-digits
        (fn [x]
          (->> 0
            (iterate #(+ x %))
            (drop-while (complement two-or-three-digit-number?))
            (take-while two-or-three-digit-number?)
            (map utils/digits)
            (map (fn [x] (if (= 3 (count x))
                           x
                           (cons 0 x)))) 
            (filter unique-digits?)))
        join-from (fn [[a b & _ :as digit-seq] potential-digits-to-add]
                    (->> potential-digits-to-add
                      (filter (fn [[x y z :as coll]]
                                (and (= y a) (= z b) (not (some #{x} digit-seq)))))
                      (map (fn [[x]] (cons x digit-seq)))))
        potential-multiples (map multiples-of-x-with-unique-digits [17 13 11 7 5 3 2])]
    (->> (reduce (fn [xs ys]
                   (mapcat (fn [x] (join-from x ys)) xs))
           (first potential-multiples) (rest potential-multiples))
      (filter #((set %) 0)) ; 0 can't be the first digit
      (map (fn [digits] (cons
                          (first (remove (set digits) (range 10)))
                          digits)))
